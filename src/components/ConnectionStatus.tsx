...
export function ConnectionStatus({ status }: ConnectionStatusProps) {
  ...
  return (
    <div className="absolute top-5 left-5 bg-white dark:bg-gray-800 px-4 py-3 rounded-xl shadow-md z-[1000] flex items-center gap-3">
      <div className="flex items-center gap-2">
        <div
          className={`w-3 h-3 rounded-full ${config.color} ${config.animate ? 'animate-pulse' : ''}`}
        />
        <span className="text-xs font-medium text-gray-700 dark:text-gray-300">{config.text}</span>
      </div>
    </div>
  );
}
